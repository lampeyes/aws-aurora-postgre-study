...
→「DBクラスター識別子」→ "ap96cXXXXX" (XXXXXは任意の文字列、3~5文字程度)を入力
→「データベースの名前」→ "mydb"
→「削除保護」チェックボックスを外す
→「データベースを作成」ボタンをクリック
→「DBインスタンスの詳細表示」をクリック→「インスタンスタブ」リンクをクリック
```
...
```
「接続」ペイン
→「セキュリティグループ」の「タイプ」が  "Inbound" となっている
  セキュリティグループのリンクをクリックしてEC2ダッシューボードページへ移動
```

```
「セキュリティグループ」ペインの「インバウンド」タブ→「編集」ボタン
→「インバウンドルールの編集」ダイアログの「タイプ」が"PostgreSQL"となっているルールの「ソース」列下の
　ドロップダウンメニューをクリック
→「マイIP」を選択→「保存」ボタンをクリック
```
...
## 2-2. データベースへの接続確認
### 2-2-1 接続設定の作成
* PC常にインストールされている **Psqledit** を起動します。 **クラスターエンドポイント向け接続設定** と **読み込みエンドポイント向け接続設定** の２つの設定を作成してください。
* 接続設定に必要な情報は以下の通りです。
...
```
USER    : awsuser
PASS    : **"2-1. Aurora PostgreSQLクラスターの作成"で設定した「マスターパスワード」**
DBNAME  : **"2-1. Aurora PostgreSQLクラスターの作成"で設定した「データベースの名前」**
HOST    : **"2-1. Aurora PostgreSQLクラスターの作成"で確認したクラスターエンドポイント
PORT    : 5432
OPTION  : 未入力
NAME    : クラスターエンドポイント
...
PASS    : **"2-1. Aurora PostgreSQLクラスターの作成"で設定した「マスターパスワード」**
DBNAME  : **"2-1. Aurora PostgreSQLクラスターの作成"で設定した「データベースの名前」**
HOST    : **"2-1. Aurora PostgreSQLクラスターの作成"で確認した読み込みエンドポイント
PORT    : 5432
OPTION  : 未入力
NAME    : 読み込みエンドポイント
...
```
Psqleditの「login」ダイアログの「接続先の選択」ペインから作成した　
**クラスターエンドポイント接続設定**　を選択
→「OK」ボタンをクリック→「SQL編集」ペイン
```
...
```
Psqleditのメニュー→「ファイル」→「接続先の変更」→「login」ダイアログの「接続先の選択」ペインから
**読み込みエンドポイント向け接続設定** をダブルクリック
```
* 以下のクエリーを「SQL編集」ペインへ入力後 ▶︎(SQL実行ボタン)をクリックし正常に実行されるか確認してください。
```
select version();
...
→「レプリカの作成」ベイン→「設定」ペイン→"aup96-handson-replica2"を入力
→「Auroraレプリカの作成」をクリック
```
* 追加作成したレプリカのステータスが、 **”利用可能”** に変わるのを待ちます。（6分程度）
* レプリカのステータスが **"利用可能"**　になったところで、先ほど取得したスナップショット **"handson-aurora-snapshot"** のステータスが **利用可能** となっているか確認します。
```
「サイドバー」→「スナップショット」
```

* 追加したDBインスタンス"aup96-handson-replica2"がレプリカリストに追加され、ロールが **"読み込み"** になっていることが確認できます。
```
「サイドバー」→「クラスター」→「クラスター」ペイン→「DBクラスター識別子」のリンクをクリック
→「DBクラスターメンバー」ペイン
```


## 2-7. リストア
* 2-5. で取得したスナップショットからDBインスタンスを別クラスターとして復元(リストア)します。
* この例ではスナップショットを取得したDBインスタンスが存在しているため別DBインスタンスとして復元します。スナップショットを取得したDBインスタンスが削除済みである場合には、削除前のDBインスタンス識別子で復元することができます。
* **特に入力・選択指示のない入力・選択項目は、設定されている値のままとしてください**

```
「サイドバー」→「スナップショット」→ 2-5.で取得したスナップショットのチェックボックをクリック
→「スナップショットのアクション」ドロップダウンメニュー→「スナップショットの復元」を選択
→「DBインスタンスクラス」ドロップダウンメニュー
→"db.r4.large - 2 vcpu, 15.25 GiB RAM"を選択
→「DBインスタンス識別子」→"ap96rXXXXX" (XXXXXは任意の文字列、3~5文字程度)を入力
→「DBインスタンスの復元」ボタンをクリック
```

* 復元したDBインスタンスがのステータスが **利用可能** になるまで待ちます。（15分程度）
* スナップショットから復元されるDBインスタンスは1インスタンスのみです。
* プライマリーインスタンスのみが含まれるクラスターが復元されるため、レプリカが必要な場合には別途追加する必要があります。
* 復元では、DBパラメータグループやDBクラスターのパラメータグループをカスタマイズしている場合にはカスタマイズ済みのパラメータグループを選択する必要があります。
* 可用性向上のため適切なアベイラビリティーゾーンを選択する必要があります。
* クラスターの停止・開始機能のリリースにより、開発環境など利用しない時間帯にクラスターの停止開始の代替操作としてのクラスターの削除、リストア、レプリカ追加という煩雑な操作は不要になりました。

## 2-8. クラスターの停止
* **"2-1. Aurora PostgreSQLクラスターの作成"** で作成した **"ap96c"** で始まるクラスター識別子をもつクラスター(3 DBインスタンス)を使って **停止・削除** を試します。

```
「サイドバー」→「クラスター」
→ "ap96c" で始まるDBクラスター識別子横にあるラジオボタンをクリック
→「クラスターアクション」→「停止」→「データベースの停止」ボタンをクリック
```
...
* クラスターの開始を待つ間に、リストアで作成したクラスターを削除します。


## 2-10. クラスター内の全DBインスタンスの削除（兼　環境クリーンアップ）
* DBインスタンンスを削除します

* 最初に **"ap96r"** で始まるクラスター（リストアで作成したクラスター）のDBインスタンスを削除します。
```
「サイドバー」→「インスタンス」
→　"ap96r" で始まるDBインスタンス横にあるラジオボタンをクリック
→ 「インスタンスの操作」メニュー
→ 「最終スナップショットを作成しますか？ドロップダウンメニュー」→”いいえ”を選択
→ 「インスタンスの削除後、システムスナップショットとポイントインタイムの復元を含む自動バックアップが利用不可となることを了承しました。」チェックボックスをチェック
→ テキストボックスへ  delete me  を入力→「削除」ボタンをクリック
```
* "ap96r" で始まるDBインスタンスのステータスが **"削除中"** に変わります。
* **ここで一旦、DBインスタンス削除操作は止めてください。 2-9. クラスターの開始で開始中のクラスターが起動するのも待ちます**

* 2-9. クラスターの開始起動中のDBインスタンス全てのステータスが **"利用可能"** となった後、以下の操作を行います。
* プライマリインスタンス、レプリカの順で削除します。
```
「サイドバー」→「インスタンス」
→　DBインスタンス横のラジオボタンをクリック
→ 「インスタンスの操作」メニュ→「削除」を選択→ 「テキストボックス」へ  delete me  を入力
→ 「削除」ボタンをクリック
```

* クラスターに含まれる **最後のDBインスタンスを削除する場合** 、以下の手順で削除します。（クラスター内に含まれる最後のDBインスタンス削除時のダイアログで入力する内容が多くなります。）
```
「サイドバー」→「インスタンス」
→ DBインスタンス横のラジオボタンをクリック→「インスタンスの操作」メニュー→「削除」を選択
→ 「最終スナップショットを作成しますか？ドロップダウンメニュー」→”いいえ”を選択
→ 「インスタンスの削除後、システムスナップショットとポイントインタイムの復元を含む自動バックアップが利用不可となることを了承しました。」チェックボックスをチェック
→ 「テキストボックス」へ  delete me  を入力→「削除」ボタンをクリック
```
