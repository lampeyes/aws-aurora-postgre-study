...
→「スナップショットのアクション」→「スナップショットの復元」を選択
...
* 復元ではレプリカは別途追加する必要があります。
* クラスターの停止・開始機能のリリースにより、開発環境など利用しない時間帯にクラスターの停止開始の代替操作としてのクラスターの削除、リストア、レプリカ追加という煩雑な操作は不要になりました。

## 2-8. クラスターの停止
* 2-6.のレプリカ追加操作が完了していることを確認した後、 **"2-1. Aurora PostgreSQLクラスターの作成"** で作成したクラスターを使って **停止・削除** を試します。

```
「サイドバー」→「クラスター」
→DBクラスター横のラジオボタンをクリックして停止するクラスターを選択
→「クラスターアクション」→「停止」→「データベースの停止」を選択
```
...
* クラスターに含まれる最後のDBインスタンスを削除する場合、以下の手順で削除します。（クラスター内に含まれる最後のDBインスタンス削除時のダイアログで入力する内容が多くなります。）
```
「サイドバー」→「インスタンス」
→DBインスタンス横のラジオボタンをクリックして削除するDBインスタンスを選択→「インスタンスの操作」
