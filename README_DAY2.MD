...
　```
  FreeLocalStorage
　```
  * 入力後 ↩︎[CR] → リストされた **ap96** の 3 DBインスタンスのチェックボックスをクリック → 「ウィジェットの作成」ボタンをクリック → 「ダッシュボードの保存」ボタンをクリック
...
### 5-3-1. 遅延分析及びチューニング　その１　
  ...
  * **注意** 「メニュー」→「SQL Commander」→ 「Database」は **mydb** 、 「Schema」は、 **hoge** 　になっていることを確認、なっていない場合には、各ドロップダウンメニューより選択
...
### 5-3-2. 遅延分析及びチューニング　その２
  ...
  * DbVisualizer → 「Databases」タブ → 「Connections」ツリー → 「Cluster Endpoint」を選択、右クリック → ポップアップメニュー「Connect」
  * DbVisualizer → 「メニュー」→「SQL Commander」で、SQL Commanderペインを開く
  * DbVisualizer → 「メニュー」→「SQL Commander」→ 「Transaction」→　サブメニュー「**Turn off auto commit**」を選択する（**2つのDbVisualizerともAuto Commitはoffにする。なお、SQL Commanderの中段に、Auto Commit : ON/OFF が表示されているので合わせて確認してください**）

  * 1つめのDbVisualizerで以下のSQL文を実行
  ```
  update t1 set description = 'hoge' where ref_no = 1;
  ```
  * **Auto Commit OFFの場合、"Commit"/"Rollback"/"Continue with Uncommited"を問うダイアログが表示されます。 "Continue with Uncommited"ボタンをクリックしてください。**
...
### 5-3-3. 遅延分析及びチューニング　その３ （オプション）
  ...
  * 1つ目の DbVisualizer で以下の無名PL/pgSQLスクリプトをSQL Commander へコピーペースト → 「メニュー」→「SQL Commander」→「Execute Buffer」（▶︎アイコンと赤い●アイコンが組み合わされている）を洗濯して実行します。（このスクリプトは無限ループし1vCPUのコアを消費し続けます
  ```
  DO $$BEGIN
    LOOP
      --
  ...
  * **プライマリインスタンスのDBインスタンス名を確認**
    ...
    * 2つ目の DvVisualizer で以下のSQL文を何度か実行してください。(1 行ヒットするまで実行して見てください) **Schema は public** になっていることを確認してください。
    ```
    SELECT
      pid
    ...
    * このクエリーでは実行時間が10秒以上経過したSQL文を取得するようにしています。該当クエリーが長時間実行されているいわゆるかけ逃げクエリーであれば、セッションをキャンセルすることも可能です。
